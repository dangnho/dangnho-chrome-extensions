	...
	$.fn.ruler = function(options) {
		...
		if (!Modernizr.touch) {
			...
			if (settings.showCrosshair) {
				$('#lift_ruler').append(vMouse, hMouse);
				$('body').addClass('lift_ruler_enable');
			}
			// Mouse position
			if (settings.showMousePos) {
				$('#lift_ruler').append(mousePosBox);
			}
		...
		return this.each(function() {
			...
			if (settings.vRuleSize > 0) {
				...
				$(vRule).width(settings.vRuleSize).height($( document ).height()).prependTo($this);
			}
